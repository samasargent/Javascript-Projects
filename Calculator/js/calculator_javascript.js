...
function Handle_Operator(Next_Operator) {
    ...
    //If there is no value in First_Operand, now is equal to Value_of_Input
    if (First_Operand == null) {
        Calculator.First_Operand = Value_of_Input;
    } else if (operator) { //Checks if an operator already exists
        const Value_Now = First_Operand || 0;
        //If operand exists, property lookup is performed for the operator
    ...
    Calculator.Wait_Second_Operand = true; //Sets it to wait for the next operand (value)
    Calculator.operator = Next_Operator; //Operator is stored in Next_Operator
}
const Perform_Calculation = {
    //Gives instruction on what to do for each operator button
...
function Calculator_Reset() {
    Calculator.Display_Value = '0'; //Sets display back to 0
    Calculator.First_Operand = null; //Clears First_Operand
    Calculator.Wait_Second_Operand = false; //Sets it back to be ready for first input
    Calculator.operator = null; //Clears operator
}
...
keys.addEventListener('click', (event) => {
    //The target variabkle is an object that represents the element
    //that was clicked
    const { target } = event;
    ...
    Input_Digit(target.value); //Displays value that was input
    Update_Display(); //Runs the above function, to change the display
})